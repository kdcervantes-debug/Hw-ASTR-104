...
Part 2: Hydrogen EOS (H2, HI, HII)

This script:
- Builds internal partition functions for H2 and HI
- Solves for molecular equilibrium H2 <-> 2 HI
- Solves for ionization equilibrium HI <-> HII + e-
- Computes free energy and specific entropy for each species
- Computes total mixture entropy s_mix(T, P)
- Computes numerical derivatives of log s_mix
- Computes adiabatic gradient nabla_ad(T, P)
- Produces and saves plots for composition, entropy, and nabla_ad

You can post this as a GitHub repo and reference it in your homework.
"""
...
# H2 vibrational & rotational "temperatures" (approx)
...
def Z_int_HI(T, nmax=10):
    ...
    Energy zero is taken at the ground state.
    """
    Z = 0.0
    E1 = -13.6 * eV
    for n in range(1, nmax + 1):
        g_n = 2.0 * n**2
        E_n = -13.6 * eV / (n**2)
        E_rel = E_n - E1   # measure wrt ground
...
def Z_elec_H2(T):
    ...
    Ground + first excited separated by the bond energy.
    """
...
def Z_rot_H2(T, jmax=100):
    ...
    Even j: parahydrogen (g_spin = 1)
    Odd j:  orthohydrogen (g_spin = 3)
    """
    Z = 0.0
...
# ====================================================
# Equilibrium Constants K_mol(T) and K_ion(T)
# ====================================================
...
def K_mol(T):
    ...
    where n_tot is the total hydrogen number density.

    K_mol(T) ~ (lambda_HI^6 / lambda_H2^3) * (Z_HI^2 / Z_H2) * exp(-E_b/kT)
    """
    ...
    pref = (lam_HI**6 / lam_H2**3) * (ZHI**2 / ZH2)
...
def K_ion(T):
    ...
    Taking Z_HII ~ 1, Z_e = 2 (spin).
    """
    lam_e = thermal_lambda(m_e, T)
    ZHI   = Z_int_HI(T)
    pref  = (2.0 / lam_e**3) * (2.0 / ZHI)   # factor of 2 from electron spin
...
def specific_free_energy(n_s, m_s, Zint_s, T):
    ...
    F_s / N_s = k_B T [ ln(n_s lambda_s^3 / Z_int_s) - 1 ]
    f_s = (F_s/N_s) / m_s
    """
...
def centered_derivative(f, axis, dx):
    """
    Centered finite difference derivative along given axis.
    Uses periodic roll at boundaries (good enough for smooth maps).
    """
    return (np.roll(f, -1, axis=axis) - np.roll(f, 1, axis=axis)) / (2.0 * dx)


# ====================================================
# Main EOS Solver on a Grid
# ====================================================
...
def compute_eos_grid(
    ...
    """
    Compute EOS on a (log10 T, log10 P) grid.

    Returns:
        logT, logP   (1D arrays)
        xH2, xHI, xHII (2D arrays: number fractions)
        s_mix        (2D array: specific entropy [erg/g/K])
        nabla_ad     (2D array: adiabatic gradient)
    """
    # grids in log10 T and log10 P
    ...
    for iP, P in enumerate(Pvals):
        for iT, T in enumerate(Tvals):
            # total H nuclei number density
            n_tot = P / (k_B * T)

            # --- Molecular equilibrium H2 <-> 2HI ---
            Km = K_mol(T)
            A  = Km / n_tot
            ...
            n_HI  = x_atomic * n_tot
            n_H2  = (1.0 - x_atomic) * n_tot / 2.0  # factor 2: each H2 has 2 H nuclei

            # --- Ionization equilibrium HI <-> HII + e- ---
            Kion = K_ion(T)
            # n_HII^2 / n_HI = Kion  -> n_HII = sqrt(Kion * n_HI)
            n_HII = np.sqrt(max(0.0, Kion * n_HI))
            n_HII = min(n_HII, n_HI)  # cannot ionize more than available HI

            n_HI_neutral = n_HI - n_HII
            ...
            # --- Mass density (ignore electron mass) ---
            ...
            # --- Specific free energies for each species ---
            f_H2  = 0.0
            f_HI  = 0.0
            f_HII = 0.0
            ...
            if n_HII > 0.0:
                # HII internal partition ~ 1
                f_HII = specific_free_energy(n_HII, m_H, 1.0, T)

            # mixture specific free energy (mass-weighted)
            f_mix = Y_H2 * f_H2 + Y_HI * f_HI + Y_HII * f_HII

            # Approximate entropy s_mix ~ - (∂f/∂T)_rho
            # Here we use a simple proxy s ~ -f/T, good enough for smooth trends
            s_mix[iP, iT] = -f_mix / T
    ...
    # nabla_ad = - (∂ log s / ∂ log P)_T / (∂ log s / ∂ log T)_P
...
def plot_composition(logT, logP, xH2, xHI, xHII, outdir):
    """
    Plot three-panel composition: H2, HI, HII.
    """
...
def plot_nabla_ad(logT, logP, nabla_ad, outdir):
    """
    Plot adiabatic temperature gradient nabla_ad.
    """
    extent = [logT.min(), logT.max(), logP.min(), logP.max()]
